...
int nvgCreateImage(NVGcontext* ctx, const char* filename, int imageFlags)
{
	int w, h, n, image;
	unsigned char* img;
	_stbi_set_unpremultiply_on_load(1);
	_stbi_convert_iphone_png_to_rgb(1);
	img = _stbi_load(filename, &w, &h, &n, 4);
	if (img == NULL) {
//		printf("Failed to load %s - %s\n", filename, _stbi_failure_reason());
		return 0;
	}
	image = nvgCreateImageRGBA(ctx, w, h, imageFlags, img);
	_stbi_image_free(img);
	return image;
}
...
{
	int w, h, n, image;
	unsigned char* img = _stbi_load_from_memory(data, ndata, &w, &h, &n, 4);
	if (img == NULL) {
//		printf("Failed to load %s - %s\n", filename, _stbi_failure_reason());
		return 0;
	}
